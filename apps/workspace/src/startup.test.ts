...
describe("startup validation matches the runtime (audit #10)", () => {
  it("isLoopbackBind accepts only NUMERIC loopback — resolver names are not literals", () => {
    for (const good of ["127.0.0.1", "127.0.0.53", "127.255.255.254", "::1"]) {
      expect(isLoopbackBind(good), good).toBe(true);
    }
    for (const bad of [
      "localhost", // the hosts file or DNS decides what this means
