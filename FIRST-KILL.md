...
pnpm install     # installs, then BUILDS the workspace (root `prepare`, ~15s)
```

The warnings about bins it could not create are expected: they point at
`dist/` files the build creates moments later, and they are gone on the next
install. Everything below needs that build — the packages import each
other's build output, so an unbuilt tree fails with
`ERR_MODULE_NOT_FOUND … /dist/index.js`, which reads like a broken repo and
is really just a missing `pnpm build`.
...
If it answers `port 4600 is already in use`, one is still running from an
earlier attempt — keep that one, or start this on another port with
`OWNERSWITCH_CONTROL_PLANE_PORT=4601` **and** its own
...
Every line must be `✔` — each failing line says exactly what to fix. One of
them, **startup gates**, runs the checks the gateway itself makes before it
serves anything: those refuse at startup, and an MCP client shows a refusing
gateway only as a closed connection, so they are caught here instead.
...
```bash
curl -fsS -X POST http://127.0.0.1:4600/kill -d '{"reason":"first kill tutorial"}'
# → {"killed":true,"epoch":1}
```
...
Rush GO 2/2 and you get `409 {"error":"restore rejected"}` — and that is all
you get, whatever went wrong. The body never says which check failed (wrong
owner, wrong epoch, too early, replayed), because that answer would be a map
for someone who is not the owner. The `cooldownRemainingMs` above is the
number to read; asking for it requires your token. (The `-fsS` above makes a
failed step fail loudly rather than printing an error body that scrolls past
